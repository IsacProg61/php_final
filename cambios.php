...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
    try {
        $stmt = $pdo->prepare("UPDATE recetario SET nombre=?, descripcion=?, ingredientes=?, instrucciones=?, categoria=?, tiempo_prep=?, porciones=?, dificultad=?, imagen_url=? WHERE id=?");
        $stmt->execute([
            $_POST['nombre'], $_POST['descripcion'], $_POST['ingredientes'], 
            $_POST['instrucciones'], $_POST['categoria'], $_POST['tiempo_prep'], 
            $_POST['porciones'], $_POST['dificultad'], $_POST['imagen_url'], $_POST['id']
        ]);
        $mensaje = '<div class="alert alert-success">✅ Receta actualizada exitosamente.</div>';
    } catch(Exception $e) {
...
<head>
    ...
    <title>Cambios - Mi Recetario</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;700&display=swap" rel="stylesheet">
    <style>
        :root { --primary: #f59e0b; --primary-hover: #d97706; --bg-color: #0f172a; --card-bg: rgba(30, 41, 59, 0.7); --text-main: #f8fafc; --text-muted: #94a3b8; }
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .navbar h1 { font-size: 24px; font-weight: 700; background: linear-gradient(to right, #fbbf24, #f59e0b); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        .list-item:hover { border-color: rgba(245, 158, 11, 0.4); transform: translateX(5px); }
        ...
        .btn-edit { background: var(--primary); color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; transition: all 0.3s; text-decoration: none;}
        .btn-edit:hover { background: var(--primary-hover); transform: translateY(-2px); box-shadow: 0 4px 15px rgba(245, 158, 11, 0.3); }
        ...
        .modal-content { background: #1e293b; width: 100%; max-width: 600px; border-radius: 20px; padding: 30px; border: 1px solid rgba(255,255,255,0.1); transform: scale(0.9); transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); max-height: 90vh; overflow-y: auto; }
        .modal-backdrop.active .modal-content { transform: scale(1); }
        .modal-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px; }
        .modal-title { font-size: 24px; font-weight: 700; }
        .close-btn { background: transparent; border: none; color: var(--text-muted); font-size: 24px; cursor: pointer; transition: color 0.3s ease; }
...
<body>
    <nav class="navbar">
        <h1>Mi Recetario - Cambios</h1>
        <a href="first.php" class="btn-back">⬅ Volver al Menú</a>
    </nav>

    <div class="container">
        <h2 style="font-size: 28px; margin-bottom: 30px; font-weight: 700;">Selecciona una receta para modificar</h2>
        <?php echo $mensaje; ?>
        ...
        <div class="list-group">
            ...
            <div class="list-item">
                ...
                <button class="btn-edit" onclick='openEditModal(<?php echo json_encode($receta); ?>)'>Editar Receta</button>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="imagen_url" id="formImagen" class="form-control" required>
                </div>
                <div class="form-group">
                    <label>Descripción Corta</label>
                    <input type="text" name="descripcion" id="formDesc" class="form-control" required>
                </div>
                <div class="form-row">
                    <div class="form-group">
                        <label>Categoría</label>
                        <input type="text" name="categoria" id="formCat" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Dificultad</label>
                        <select name="dificultad" id="formDif" class="form-control" required>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="tiempo_prep" id="formTiempo" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Porciones</label>
                        <input type="number" name="porciones" id="formPorciones" class="form-control" required>
                    </div>
                </div>
                <div class="form-group">
                    <label>Ingredientes</label>
                    <textarea name="ingredientes" id="formIng" class="form-control" required></textarea>
                </div>
                <div class="form-group">
                    <label>Instrucciones</label>
                    <textarea name="instrucciones" id="formInst" class="form-control" required></textarea>
                </div>
                <div style="display: flex; gap: 15px; margin-top: 30px;">
                    <button type="button" class="btn-back" style="flex:1" onclick="closeModal()">Cancelar</button>
                    <button type="submit" class="btn-edit" style="flex:1">Guardar Cambios</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(data) {
            ...
            document.getElementById('formImagen').value = data.imagen_url;
            document.getElementById('formDesc').value = data.descripcion;
            document.getElementById('formCat').value = data.categoria;
            document.getElementById('formDif').value = data.dificultad;
            document.getElementById('formTiempo').value = data.tiempo_prep;
            document.getElementById('formPorciones').value = data.porciones;
            document.getElementById('formIng').value = data.ingredientes;
            document.getElementById('formInst').value = data.instrucciones;
